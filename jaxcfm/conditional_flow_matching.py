...
def pad_t_like_x(t, x):
    ...
    Parameters
    ----------
    x : Array, shape (bs, *dim)
        represents the source minibatch
    t : Array, shape (bs)

    Returns
    -------
    t : Array, shape (bs, number of x dimensions)

    Example
    -------
    x: Array (bs, C, W, H)
    t: Vector (bs)
    pad_t_like_x(t, x): Array (bs, 1, 1, 1)
    """
...
class ConditionalFlowMatcher:
    ...
    def sample_xt(self, key, x0, x1, t, epsilon):
        """
    ...
    def sample_location_and_conditional_flow(self, key, x0, x1, t=None, return_noise=False):
        ...
        xt : Array, shape (bs, *dim)
            represents the samples drawn from probability path pt
        ut : conditional vector field ut(x1|x0) = x1 - x0
        (optionally) eps: Array, shape (bs, *dim) such that xt = mu_t + sigma_t * epsilon
        ...
        if t is None:
            ...
            t = random.uniform(subkey, (x0.shape[0],), minval=0.0, maxval=1.0, dtype=x0.dtype)
        assert len(t) == x0.shape[0], "t has to have batch size dimension"

        key, subkey = random.split(key)
        eps = self.sample_noise_like(subkey, x0)
        xt = self.sample_xt(key, x0, x1, t, eps)
...
class ExactOptimalTransportConditionalFlowMatcher(ConditionalFlowMatcher):
    ...
    def sample_location_and_conditional_flow(self, key, x0, x1, t=None, return_noise=False):
        ...
        xt : Array, shape (bs, *dim)
            represents the samples drawn from probability path pt
        ut : conditional vector field ut(x1|x0) = x1 - x0
        (optionally) epsilon : Array, shape (bs, *dim) such that xt = mu_t + sigma_t * epsilon
        ...
        """
        key, subkey = random.split(key)
        x0, x1 = self.ot_sampler.sample_plan(subkey, x0, x1)
        return super().sample_location_and_conditional_flow(key, x0, x1, t, return_noise)

    def guided_sample_location_and_conditional_flow(
        self, key, x0, x1, y0=None, y1=None, t=None, return_noise=False
    ):
        r"""
        Compute the sample xt (drawn from N(t * x1 + (1 - t) * x0, sigma))
        and the conditional vector field ut(x1|x0) = x1 - x0, see Eq.(15) [1]
        ...
        x1 : Array, shape (bs, *dim)
            represents the target minibatch
        y0 : Array, shape (bs) (default: None)
            represents the source label minibatch
        y1 : Array, shape (bs) (default: None)
        ...
        """
        key, subkey = random.split(key)
        x0, x1, y0, y1 = self.ot_sampler.sample_plan_with_labels(subkey, x0, x1, y0, y1)
        if return_noise:
            t, xt, ut, eps = super().sample_location_and_conditional_flow(key, x0, x1, t, return_noise)
            return t, xt, ut, y0, y1, eps
        else:
            t, xt, ut = super().sample_location_and_conditional_flow(key, x0, x1, t, return_noise)
...
class VariancePreservingConditionalFlowMatcher(ConditionalFlowMatcher):
    ...
    def compute_mu_t(self, x0, x1, t):
        ...
        """
        t = pad_t_like_x(t, x0)
        return jnp.cos(math.pi / 2 * t) * x0 + jnp.sin(math.pi / 2 * t) * x1
    ...
    def compute_conditional_flow(self, x0, x1, t, xt):
        ...
        del xt
        t = pad_t_like_x(t, x0)
        return math.pi / 2 * (jnp.cos(math.pi / 2 * t) * x1 - jnp.sin(math.pi / 2 * t) * x0)
